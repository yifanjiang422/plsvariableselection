dummyGPLS <- function (X, Z, ncomp, mode = "regression", max.iter = 100, tol = 1e-06, 
          keepX, keepZ = NULL, ind.block.x, ind.block.z = NULL, masses, weights) 
{
  ...
  #R <- t(X) %*% Z
  res.load <- dummyGPLSStep1(X,Z, ind.block.x = ind.block.x, 
  ...
  if (ncomp > 1) {
    for (h in 2:ncomp) {
      res.load <- dummyGPLSStep1(X = res.deflat$X.h, 
