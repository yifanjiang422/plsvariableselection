osquant <- function(RawX,RawZ,ncomp,quantscale){
  ...
  Omega<-matrix(,n,ncomp)                                   #Y Score, component in the Y space 
  Omega<-matrix(c(rep(c(1,rep(0,(n-1))),ncomp)),n,ncomp)
  U<-matrix(,p,ncomp)                                       #X weight X loading
  Xi<-matrix(,n,ncomp)                                      #X score, Latend Component as a linear component of X variables
  betaXi<-matrix(,n,ncomp)
  V<-matrix(,q,ncomp)                                       #Y Weight                            
  correc<- quantscale
  beta<-matrix(,ncomp,1)
  Pmat<-matrix(,p,ncomp)
