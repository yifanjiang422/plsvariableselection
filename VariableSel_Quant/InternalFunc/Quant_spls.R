qspls <- function (X, Y, ncomp, mode = "regression", max.iter = 500, tol = 1e-06, 
          keepX = rep(ncol(X), ncomp), keepY = rep(ncol(Z), ncomp), Omega,Xi,U,V,noiteration) 
{
  X <- as.matrix(X)
  Y <- as.matrix(Y)
  q <- ncol(Y)
  ...
  res.load <- step1.spls.sparsity(X = X.s, Y.s, sparsity.x = sparsity.x[1], 
  ...
  #res.deflat <- quant.step2.spls(X=X.s, Y=Y.s, U[,1], V[,1], Xi[,1], Omega[,1], mode=mode)
  mat.c[, 1] <- res.deflat$c
  iter <- noiteration+res.load$iter
  if (mode == "regression") 
  ...
  if (ncomp > 1) {
    for (h in 2:ncomp) {
      res.load <- step1.spls.sparsity(X = res.deflat$X.h, 
                                      res.deflat$Y.h, sparsity.x = sparsity.x[h], sparsity.y = sparsity.y[h], 
                                      epsilon = tol, iter.max = max.iter)
  ...
  result <- list(call = cl, X = X.s, Y = Y.s, ncomp = ncomp, 
                 ...
                 tol = tol, max.iter = max.iter, iter = iter)
